import {
  ...
  trendSignals,
  userProfile,
  wardrobeItems as staticWardrobeItems,
...
function getItem(id: string, wardrobe: WardrobeItem[]) {
...
function pickItems(base: WardrobeItem, wardrobe: WardrobeItem[]) {
  const bottoms = wardrobe.filter(
    (item) => item.category === "bottom" && item.id !== base.id,
  );
  const topsAndLayers = wardrobe.filter(
    (item) =>
  ...
  const fallbackShoe = getItem("white-sneakers", wardrobe);
  const fallbackLayer = getItem("cream-tee", wardrobe);
  const watch = accessories[0] ?? getItem("silver-watch", wardrobe);
  ...
  const firstBottom = rankedBottoms[0]?.item ?? fallbackBottom;
  const firstShoe = rankedShoes[0]?.item ?? fallbackShoe;
...
export function buildRecommendations(
  ...
  const wardrobe = customWardrobe && customWardrobe.length > 0
    ? customWardrobe
    : staticWardrobeItems;

  const base = getItem(selectedItemId, wardrobe) ?? wardrobe[0];
  if (!base) return [];
  ...
  return alternatives
    .map((targetOccasion, index) => {
      const items = pickItems(base, wardrobe);
...
export function buildOutfitOfTheDay(customWardrobe?: WardrobeItem[]) {
  // Always fall back to static wardrobe so we have a non-null result
  const wardrobe =
    customWardrobe && customWardrobe.length > 0 ? customWardrobe : staticWardrobeItems;
...
export function generateOutfits(
  ...
  return seeds.map(({ itemId, occasion }) => {
    const rec = buildRecommendations(itemId, occasion, wardrobe)[0];
    if (!rec) return null;
    return {
  ...
  }).filter((x): x is GeneratedOutfit => x !== null);
}

export function buildChatReply(prompt: string, customWardrobe?: WardrobeItem[]) {
  ...
  if (normalizedPrompt.includes("white sneakers") || normalizedPrompt.includes("white shoes")) {
    return {
      ...
      recommendations: buildRecommendations("white-sneakers", "campus", customWardrobe),
    };
  }
  ...
  if (
    ...
    return {
      ...
      recommendations: buildRecommendations("black-hoodie", "streetwear", customWardrobe),
    };
  }
  ...
  if (
    ...
    return {
      ...
      recommendations: buildRecommendations("cream-tee", "campus", customWardrobe),
    };
  }
  ...
  if (
    ...
    return {
      ...
      recommendations: buildRecommendations("navy-knit", "travel", customWardrobe),
    };
  }
  ...
  if (
    ...
    return {
      ...
      recommendations: buildRecommendations("navy-knit", "night-out", customWardrobe),
    };
  }
  ...
  if (
    ...
    return {
      ...
      recommendations: buildRecommendations("charcoal-trouser", "smart-casual", customWardrobe),
    };
  }
  ...
  if (normalizedPrompt.includes("olive") || normalizedPrompt.includes("earth")) {
    return {
      ...
      recommendations: buildRecommendations("olive-overshirt", "smart-casual", customWardrobe),
    };
  }
  ...
  return {
    message: `I can help with prompts like ${promptSuggestions
      .map((entry) => `"${entry}"`)
      .join(", ")}.`,
    recommendations: buildRecommendations(undefined, undefined, customWardrobe),
  };
}
